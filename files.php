...
function bm_check_files_and_import() {
   ...
   // PRODUCTS
   $product_files = new BM_XML_Product_Files();
   
   $files_to_proccess = $product_files->files_to_proccess;

   if ($files_to_proccess) {
      foreach($files_to_proccess as $file) {
         error_log('proccessing file: '. $file['name']);
         $import = new BM_XML_Products_Import($file['file_path']); // import XML
      ...
      error_log('no new products files to import');
   }
   ...
   // ORDERS
   $order_files = new BM_XML_Order_Files();
   
   $files_to_proccess = $order_files->files_to_proccess;

   if ($files_to_proccess) {
      foreach($files_to_proccess as $file) {
         error_log('proccessing file: '. $file['name']);
         $import = new BM_XML_Orders_Import($file['file_path']); // import XML
      ...
      error_log('no new order files to import');
   }
